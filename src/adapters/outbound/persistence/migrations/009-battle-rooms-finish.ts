...
import { up as restoreSkillsValidator } from './008-battle-rooms-skills'

/**
 * Amplia el validador `$jsonSchema` de `battle-rooms` para la finalizacion de la
 * batalla (HU-21, RF-21): el estado `FINISHED`, el `result` unico de la sala, el
 * inicio del turno vigente (`battle.turnStartedAt`) y los eventos `turnTimedOut`
 * y `battleFinished`.
 *
 * ADITIVA Y RETROCOMPATIBLE, NO DESTRUCTIVA -- MISMO CRITERIO que `003`..`008`:
 * los campos nuevos son OPCIONALES (ninguno entra en `required`) porque los
 * documentos escritos por `008` no los tienen y MongoDB no revalida en reposo lo
 * ya almacenado. Ningun documento necesita *backfill*: una sala anterior a HU-21
 * se restaura con `result: null` y sin `turnStartedAt` (su turno empezo con la
 * batalla, contrato §12).
 *
 * `001`..`008` quedan CONGELADAS: esta es una migracion nueva. Es AUTOCONTENIDA
 * (no importa las constantes de `008`) para que una futura edicion de aquella no
 * cambie en silencio lo que esta aplica. El numero `009` es el siguiente libre.
 *
...
/** Vuelve al validador de `008`. Las salas `FINISHED` ya escritas no se reescriben. */
export const down = async (db: Db): Promise<void> => {
  await restoreSkillsValidator(db)
}
