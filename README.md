...
**Implementada la resolución `RandomIndex` (HU-24) → fila → efecto y magnitud**: tabla de 8000 filas por tipo de héroe (Tabla 21 del documento oficial: Guerrero Tanque/Armas, Mago Fuego/Hielo, Pícaro Veneno/Machete), mecánica de modificadores (+6 % de crítico compensado desde «no causar daño», Tabla 23) y caso de uso `ResolveRandomEffect`. Además, `BuildHeroEffectTable` construye la **tabla vigente del héroe equipado real**: parte de la tabla base de su `subtype` (Player-Inventory) y aplica los efectos de su equipamiento (`activeEffects`) que tienen semántica definida. **Solo `CRITICAL_CHANCE INCREASE PERCENTAGE` incondicional, permanente y sobre uno mismo modifica la tabla** (100 pb = +1 punto porcentual absoluto, restado de «no causar daño»; +600 pb reproduce la Tabla 23); el resto de efectos se clasifica y `pendingEffects` declara los que podrían afectarla y **no** se aplicaron. **Aún no la invoca ningún flujo** (HU-20 deberá hacerlo cuando Ataque > Defensa), no calcula daño numérico y **no hay endpoint público**. Pendientes: Chamán/Médico sin tabla válida en el documento, selección concreta del crítico 120–180 % y los efectos de equipamiento sin semántica formal (condicionados, temporales, hacia otro objetivo, otras operaciones o magnitudes). Ver [docs/hu-25-effect-control-table.md](docs/hu-25-effect-control-table.md).
