...
**Implementada la resolución `RandomIndex` (HU-24) → fila → efecto y magnitud**: tabla de 8000 filas por tipo de héroe (Tabla 21 del documento oficial: Guerrero Tanque/Armas, Mago Fuego/Hielo, Pícaro Veneno/Machete), mecánica de modificadores (+6 % de crítico compensado desde «no causar daño», Tabla 23) y caso de uso `ResolveRandomEffect`. Además, `BuildHeroEffectTable` construye la **tabla base del héroe equipado real** (`subtype` que entrega Player-Inventory) y clasifica los efectos de su equipamiento (`activeEffects`); `pendingEffects` declara los que **no** están en la tabla. **Aún no la invoca ningún flujo** (HU-20 deberá hacerlo cuando Ataque > Defensa), **ningún efecto de equipamiento modifica todavía la tabla**, no calcula daño numérico y **no hay endpoint público**. Pendientes: Chamán/Médico sin tabla válida en el documento, selección concreta del crítico 120–180 % y **la unidad de `CRITICAL_CHANCE` con `PERCENTAGE`** (+3 puntos absolutos o +3 % relativo), que ningún requisito define. Ver [docs/hu-25-effect-control-table.md](docs/hu-25-effect-control-table.md).
