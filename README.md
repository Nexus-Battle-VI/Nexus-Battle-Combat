...
**Implementado** el inicio de batalla desde una sala `PREPARING` (`POST /api/v1/combat/rooms/{roomId}/start`, idempotente, revalida la elegibilidad precombate de HU-16), la **cola de turnos inmutable** generada con el motor de HU-24 (muestreo por rechazo, sin sesgo; alternancia de equipos; sin estadísticas), el avance de turno **del lado del servidor** (`CompleteBattleTurn`, sin ruta pública), `battleStarted`/`turnAdvanced` con `seq`, y el WebSocket de ADR-020 completo (**ticket de un solo uso**, `resume`/`snapshot`, latido, 16 KiB). La batalla se guarda en el mismo documento de la sala (migración `005`). Sin ataque, daño, habilidades ni fin de batalla (HU-18, HU-19, HU-21). Semilla: secuencia de proceso con la semilla validada por HU-26 (`COMBAT_RANDOM_SEED`, por defecto 3.000.000); **no** es una política de semilla por batalla. Ver [docs/hu-17-turn-order.md](docs/hu-17-turn-order.md).
