...
| Moderación                | Censura server-side con `#` del vocabulario ofensivo curado, antes de persistir y difundir (palabra completa, tolerante a tildes, mayúsculas, repeticiones, sustituciones numéricas y letras separadas); el mensaje no se rechaza | `ChatProfanityPolicy` / `ChatOffensiveTerms` |
...
- **Moderación limitada a censura.** El vocabulario ofensivo se reemplaza por `#` en el servidor (`ChatProfanityPolicy`); no hay reportes ni sanción automática, y los mensajes anteriores a la censura no se reescriben (caducan con la retención).
