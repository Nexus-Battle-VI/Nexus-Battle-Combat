...
Estado terminal `FINISHED` con un **resultado único** (`BattleResult`, validado por `parseBattleResult`). `BattleRoom.finish` es idempotente y solo admite `IN_BATTLE`; `applyBasicAttack`/`applySkill` llaman a `concludeIfEliminated` al final, de modo que un golpe letal deja la acción y `battleFinished` en la **misma escritura** con `seq` contiguos. `BattleRoom.settleDeadlines` (puro, una transición por llamada) decide el vencimiento más antiguo (gracia > global > turno) y `BattleDeadlineSettler` es el único que guarda (`rooms.save`) esas transiciones, publicando **después** de persistir. Los comandos liquidan sus vencimientos antes de validar (`ExecuteBasicAttack`/`UseSkill`), y `ProcessBattleDeadlines` + `IntervalBattleDeadlineScheduler` (tick de 1 s, `RecoverBattleDeadlines` al arrancar) cubren lo que nadie toca. `BattleFinalizer` ejecuta en orden fijo y sin lanzar: cancelar vencimientos, olvidar presencia, notificar al lobby, liberar conexiones (sin cerrar sockets) y publicar la notificación a consumidores (solo un registro `battle_finished`; los créditos del §7.6 viajan como derecho, no se acreditan). La presencia y el libro de vencimientos viven en memoria (ADR-020, una réplica). Migración `009` (aditiva). Detalle en [hu-21-battle-finish.md](hu-21-battle-finish.md).
...
Los vencimientos de HU-21 se resuelven con **dos mecanismos**: un barrido en el proceso (`IntervalBattleDeadlineScheduler`, cada 1 s, `tick()` público para pruebas) sobre un libro en memoria, y **liquidación perezosa** en cada comando de combate. Los vencimientos globales y de turno son **derivables del estado persistido**, así que un reinicio los recupera (`RecoverBattleDeadlines`); la **gracia de desconexión** no se persiste (es de la conexión) y al arrancar todos los participantes humanos empiezan su gracia. Las constantes (6 min, 30 s, 30 s) viven en `BattleTimingPolicy`, sin variables de entorno; el tiempo sale siempre de `ClockPort`. En pruebas el planificador se usa con `autoStart: false` y el reloj se mueve a mano. `BattleFinalizer` cancela el registro de la sala al finalizar.
...
- HU-21 (fin de batalla) está implementada; ver `docs/hu-21-battle-finish.md`. Pendientes del PO: el empate total (`NO_WINNER`), el ganador por desconexión, los créditos como derecho sin acreditar, el turno vencido como turno perdido y el chat cerrado al finalizar. El transporte de la notificación a HU-22/23/30/09 y a HU-29 es de esas historias.
