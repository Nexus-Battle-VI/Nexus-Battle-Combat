...
**Corrección (2026-09-24, hallada en producción):** el `operationId` que viaja **no es** el id lógico del workflow. El contrato de HU-59 exige UUID v1-5 (`@IsUUID()` en el DTO y `UUID_PATTERN` en el caso de uso de Player-Inventory) y el id lógico `battle:{battleId}:player:{playerId}:chest:1:grant` no lo es: Player-Inventory respondía `400` (`operationId must be a UUID`) a **todo** cofre, antes de llegar al controlador, y el flujo quedaba reintentándose para siempre. La prueba de Player-Inventory (#40) que "aceptaba a combat" usaba un UUID de ejemplo, por eso nadie lo vio. `PlayerInventoryGrantHttpClient` traduce al enviar (`toInventoryGrantOperationId`): UUID v5 determinista del id lógico, con un espacio de nombres fijo (`INVENTORY_GRANT_NAMESPACE`, **no cambiarlo nunca**: es parte de la clave de idempotencia). El id lógico sigue siendo el que se persiste en `inventoryOperationId` y el que aparece en los registros; un workflow ya persistido se recupera sin tocar sus datos. Wallet **no** se traduce: acepta el id lógico como `text`.
...
Ningún fallo revierte un crédito ya acreditado por Wallet (HU-22 §66). Un fallo transitorio (5xx, timeout, red) dobla como reintentable: no cambia de estado, queda para el siguiente barrido. Un rechazo terminal (409 con payload distinto, 422, **o cualquier 4xx permanente**) mueve el workflow a `TERMINAL_FAILURE`: no se reintenta solo.

**Clasificación de `postInternalJson`** (corregida el 2026-09-24; antes todo lo que no fuera 200/409/422 ni 401/403 era `error_servidor`, es decir, transitorio):

| Respuesta                                  | Tratamiento                                                                                                                                                           |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 200 / 409 / 422                            | `ok` / `conflict` / `rejected` (sin cambios)                                                                                                                          |
| 4xx **permanente** (400, 410, 413, 415...) | `invalid` → `RewardInvalidRequestError` → `TERMINAL_FAILURE` al primer intento. `failureReason` lleva el mensaje de validación del destino (acotado a 200 caracteres) |
| 401, 403                                   | Transitorio (`no_autorizado`): el secreto o el reloj pueden corregirse                                                                                                |
| 404, 405                                   | Transitorio: el destino aún no tiene la ruta (orden de despliegue, contrato §14)                                                                                      |
| 408, 425, 429, 5xx, red                    | Transitorio                                                                                                                                                           |

**Reintentos acotados** (`RewardRetryPolicy`): tras un fallo transitorio el workflow espera antes de volver a intentarse (1 s, 2 s, 4 s... hasta 5 min) y, tras 100 fallos acumulados (~7,7 h), pasa a `TERMINAL_FAILURE` con `failureReason` "reintentos agotados". Se deriva de `attempts` y `updatedAt`, que ya se persisten: la colección tiene un validador `$jsonSchema` con `additionalProperties: false`, así que añadir campos exigiría una migración. Las cifras son una decisión de operación (ni el contrato ni la HU las fijan): se cambian en `DEFAULT_REWARD_RETRY_POLICY`. El HU-23 (apuesta) **no** adopta la clasificación de 4xx permanentes: conserva el tratamiento previo hasta decidir qué hacer con créditos retenidos.
